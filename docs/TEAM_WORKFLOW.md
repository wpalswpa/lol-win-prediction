<!-- ─────────────────────────────────────────────
  팀 4명이 한 브랜치를 안전하게 나눠 쓰기 위한 규칙.
  왜 필요한가: 누가 어느 파일을 맡는지 갈라놔야 충돌이 안 난다. 합류 절차와 배포 순서도 여기 있다.
  주로 보는 사람: 팀원 전원 — 작업 시작 전
  ───────────────────────────────────────────── -->
...
충돌은 여럿이 **같은 파일**을 만질 때 생긴다. 그래서 사람마다 손대는 파일을 갈라놨다.
...
각자 GitHub 계정이 Collaborator 로 초대돼 있다. 초대 메일의 수락을 먼저 누른 뒤 push 한다.
그래도 403 이면 `git config user.name` 으로 지금 로그인된 계정을 확인한다.
