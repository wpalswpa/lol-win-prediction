<!-- ─────────────────────────────────────────────
  DDBM 사이트에 올리는 '시스템 의도' 5종 중 하나 — 구현 범위와 산출물.
  무엇을 실제로 만들었는지.
...
[구현 범위]
tasks 의도의 A1~A9를 전부 구현했다 (2026-09-01 완료). 코드는 GitHub `wpalswpa/lol-win-prediction`, 문서는 `specs/002-ml-prediction-service/`.
...
[아직 안 한 것]
B1 게임 모르는 사람 스모크 테스트(SC-7, 완료 — 부록 보완) · B2 viz_3 경기유형 그림 라벨 수정(완료) · B3 발표 리허설 · B4 팀 서버 배포(Flask debug 끄고 WSGI)
